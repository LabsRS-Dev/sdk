...
var cb = function (value, context, argCount) {
    if (_$3.iteratee !== builtinIteratee) {
        return _$3.iteratee(value, context);
    }
    if (value == null) {
        return _$3.identity;
    }
    if (_$3.isFunction(value)) {
        return optimizeCb(value, context, argCount);
    }
    if (_$3.isObject(value) && !_$3.isArray(value)) {
        return _$3.matcher(value);
    }
...
_$3.map = _$3.collect = function (obj, iteratee, context) {
    ...
    var keys = !isArrayLike(obj) && _$3.keys(obj), length = (keys || obj).length, results = Array(length);
...
var createReduce = function (dir) {
    ...
    var reducer = function (obj, iteratee, memo, initial) {
        var keys = !isArrayLike(obj) && _$3.keys(obj), length = (keys || obj).length, index = dir > 0 ? 0 : length - 1;
        if (!initial) {
...
_$3.every = _$3.all = function (obj, predicate, context) {
    predicate = cb(predicate, context);
    var keys = !isArrayLike(obj) && _$3.keys(obj), length = (keys || obj).length;
    for (var index = 0; index < length; index++) {
        var currentKey = keys ? keys[index] : index;
...
_$3.toArray = function (obj) {
    ...
    if (isArrayLike(obj)) {
        return _$3.map(obj, _$3.identity);
    }
...
_$3.intersection = function (array) {
    ...
    for (var i = 0, length = getLength(array); i < length; i++) {
        var item = array[i];
        if (_$3.contains(result, item)) {
            continue;
        }
        var j;
        for (j = 1; j < argsLength; j++) {
            if (!_$3.contains(arguments$1[j], item)) {
                break;
            }
        }
        if (j === argsLength) {
...
var collectNonEnumProps = function (obj, keys) {
    ...
    if (_$3.has(obj, prop) && !_$3.contains(keys, prop)) {
        keys.push(prop);
    }
    while (nonEnumIdx--) {
...
_$3.mapObject = function (obj, iteratee, context) {
    iteratee = cb(iteratee, context);
    var keys = _$3.keys(obj), length = keys.length, results = {};
    for (var index = 0; index < length; index++) {
...
var createAssigner = function (keysFunc, defaults) {
    return function (obj) {
        ...
        for (var index = 1; index < length; index++) {
            var source = arguments$1[index], keys = keysFunc(source), l = keys.length;
            for (var i = 0; i < l; i++) {
                var key = keys[i];
...
deepEq = function (a, b, aStack, bStack) {
    ...
    if (areArrays) {
        ...
        if (_$3.keys(b).length !== length) {
            return false;
        }
        while (length--) {
            // Deep compare each member
            key = keys[length];
            if (!(_$3.has(b, key) && eq(a[key], b[key], aStack, bStack))) {
                return false;
            }
        }
    }
...
_$3.isEmpty = function (obj) {
    ...
    if (isArrayLike(obj) && (_$3.isArray(obj) || _$3.isString(obj) || _$3.isArguments(obj))) {
        return obj.length === 0;
    }
...
uu$$1.ConfigServer = {
    getDomain: function (useDebug) {
        if (useDebug === void 0)
            useDebug = uu$$1.enableAppConfigDebug;
...
$bc_$2.IAP = {
    NoticeCenter: window.$ ? window.$.Callbacks() : {},
    ...
    enableIAP: function (in_parms, cb) {
        ...
        try {
            ...
            parms['products'] = [];
            if (_$8.isArray(in_parms['products'])) {
                try {
                    var productIds = [];
...
$bc_$4.App = {
    ...
    getCompatibleWebkitLanguageList: function (_getType) {
        ...
        var NativeApple2WebKit_LanguageMap = {
            ...
            'fr': ['fr', 'fr-FR', 'fr-fr'],
            'de': ['de', 'de-DE', 'de-de'],
            'zh-Hans': ['zh', 'zh-CN', 'zh-cn', 'zh-Hans'],
            'zh-Hant': ['zh-TW', 'zh-tw', 'zh-Hant'],
            'ja': ['ja', 'ja-JP', 'ja-jp'],
            'es': ['es', 'es-ES', 'es-es'],
            'es-MX': ['es-MX', 'es-XL', 'es-xl'],
            'it': ['it', 'it-IT', 'it-it'],
            'nl': ['nl', 'nl-NL', 'nl-nl'],
            'ko': ['ko', 'ko-KR', 'ko-kr'],
            'pt': ['pt', 'pt-BR', 'pt-br'],
...
$bc_$5.XPCPythonHelper = {
    ...
    */
    startWebServer: function (jsonObj, successCB, failedCB) {
        ...
        // 传递参数
        var newJson = {
            'command': ' --port=' + $bc_$5.App.getServerPort(),
            'currentDirectoryPath': _json.currentDirectoryPath || '',
            'environmentDic': _json.environmentDic || {},
            'mainThread': _json.mainThread !== false
        };
